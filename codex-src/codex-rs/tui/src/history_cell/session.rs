    ...
    if is_first_event {
        ...
        let help_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(vec![
                "  ".into(),
                "/init".into(),
                " - create an AGENTS.md file with instructions for Retrace".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/status".into(),
            ...
            Line::from(vec![
                "  ".into(),
                "/permissions".into(),
                " - choose what Retrace is allowed to do".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/model".into(),
...
impl HistoryCell for SessionHeaderHistoryCell {
    ...
    fn raw_lines(&self) -> Vec<Line<'static>> {
        let mut lines = vec![
            Line::from(format!("{} (v{})", crate::version::APP_NAME, self.version)),
            Line::from(format!(
                "model: {}{}",
                self.model,
