...
/// Like [`new_reasoning_summary_block`], but with the visibility decision forced:
/// `Some(true)` always shows the block in main history (used by `/thinking show`),
/// `Some(false)` records it transcript-only (used by `/thinking hide`), and
/// `None` keeps the header-based heuristic.
pub(crate) fn new_reasoning_summary_block_with_visibility(
    full_reasoning_buffer: String,
    cwd: &Path,
    force_visible: Option<bool>,
) -> Box<dyn HistoryCell> {
    match force_visible {
        None => new_reasoning_summary_block(full_reasoning_buffer, cwd),
        Some(visible) => Box::new(ReasoningSummaryCell::new(
            String::new(),
            full_reasoning_buffer.trim().to_string(),
            &cwd.to_path_buf(),
            /*transcript_only*/ !visible,
        )),
    }
}
