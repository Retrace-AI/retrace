...
impl AppLinkView {
    ...
    fn install_confirmation_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if is_auth_suggestion {
            for line in wrap(
                if is_codex_apps_auth {
                    "Sign in to the app on Retrace in the browser window that just opened."
                } else {
            ...
            for line in wrap(
                "Complete app setup on Retrace in the browser window that just opened.",
                usable_width,
            ) {
                lines.push(Line::from(line.into_owned()));
            }
...
mod tests {
    ...
    fn auth_url_request(url: &str) -> codex_app_server_protocol::McpServerElicitationRequest {
        codex_app_server_protocol::McpServerElicitationRequest::Url {
            ...
            message: "Reconnect Google Calendar on Retrace.".to_string(),
            url: url.to_string(),
    ...
    fn installed_app_has_toggle_action() {
        ...
        assert_eq!(
            view.action_labels(),
            vec!["Manage on Retrace", "Disable app", "Back"]
        );
    }
    ...
    fn toggle_action_sends_set_app_enabled_and_updates_label() {
        ...
        assert_eq!(
            view.action_labels(),
            vec!["Manage on Retrace", "Enable app", "Back"]
        );
    }
    ...
    fn auth_suggestion_with_reason_snapshot() {
        ...
        let view = AppLinkView::new(
            AppLinkViewParams {
                ...
                is_installed: true,
                is_enabled: true,
                suggest_reason: Some("Reconnect Google Calendar on Retrace.".to_string()),
                suggestion_type: Some(AppLinkSuggestionType::Auth),
                elicitation_target: Some(suggestion_target()),
            },
            tx,
