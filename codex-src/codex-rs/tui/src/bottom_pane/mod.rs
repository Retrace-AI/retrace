...
use crate::multi_agents::LiveAgentStatus;
use crate::multi_agents::LiveAgentStatusPanel;
use crate::rampage_dashboard::RampageDashboardPanel;
...
pub(crate) struct BottomPane {
    ...
    /// Live multi-agent activity shown above the composer during collaboration modes.
    live_agent_status: LiveAgentStatusPanel,
    /// Durable Rampage / Readonly Research mission dashboard shown above the composer.
    rampage_dashboard: RampageDashboardPanel,
...
impl BottomPane {
    pub fn new(params: BottomPaneParams) -> Self {
        ...
        Self {
            ...
            rampage_dashboard: RampageDashboardPanel::new(
                frame_requester.clone(),
                animations_enabled,
            ),
    ...
    pub(crate) fn set_live_agent_status(&mut self, status: LiveAgentStatus) {
        if self.live_agent_status.set_status(status) {
            self.request_redraw();
        }
    }

    pub(crate) fn clear_live_agent_status(&mut self) {
        if self.live_agent_status.clear() {
            self.request_redraw();
        }
    }

    /// Points the Rampage mission dashboard at the durable state file for `thread_id`.
    pub(crate) fn set_rampage_dashboard_source(
        &mut self,
        codex_home: &std::path::Path,
        thread_id: &str,
    ) {
        self.rampage_dashboard.set_source(codex_home, thread_id);
        self.request_redraw();
    }
    ...
    fn as_renderable_with_composer_right_reserve(
        ...
        if let Some(view) = self.active_view() {
            ...
            if !self.rampage_dashboard.is_empty() {
                flex.push(
                    /*flex*/ 0,
                    RenderableItem::Borrowed(&self.rampage_dashboard),
                );
            }
            if !self.live_agent_status.is_empty() {
                flex.push(
                    /*flex*/ 0,
                    RenderableItem::Borrowed(&self.live_agent_status),
                );
            }
            ...
            let has_status_or_footer = self.status.is_some()
                ...
                || !self.live_agent_status.is_empty()
                || !self.rampage_dashboard.is_empty();
    ...
    pub(crate) fn render_with_composer_right_reserve(
        ...
        self.schedule_mode_animation_frame_if_needed();
    ...
    fn schedule_mode_animation_frame_if_needed(&self) {
        if let Some(delay) = self.composer.animated_collaboration_mode_indicator_delay() {
            self.request_redraw_in(delay);
        }
    }
}
...
mod tests {
    ...
    fn test_pane_with_disable_paste_burst(
        ...
        BottomPane::new(BottomPaneParams {
            ...
            placeholder_text: "Ask Retrace to do anything".to_string(),
            disable_paste_burst,
