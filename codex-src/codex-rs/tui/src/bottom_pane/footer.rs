...
//!      drop "? for shortcuts" before dropping "(tab to cycle)".
//!    - If "(tab to cycle)" cannot fit, also hide the right-side
...
impl CollaborationModeIndicator {
    pub(crate) fn is_animated(self) -> bool {
        matches!(
            self,
            CollaborationModeIndicator::ReadonlyResearch
                | CollaborationModeIndicator::AbsoluteRampage
        )
    }

    pub(crate) fn animation_frame_delay(self) -> Option<Duration> {
        self.is_animated().then(|| Duration::from_millis(80))
    }

    fn styled_spans(self, show_cycle_hint: bool) -> Vec<Span<'static>> {
        mode_tabs_spans(self, show_cycle_hint)
    }
}
...
const ASK_MODE_COLORS: [Color; 1] = [Color::Blue];
const BUILD_MODE_COLORS: [Color; 1] = [Color::Green];
const PLAN_MODE_COLORS: [Color; 1] = [Color::Magenta];
const RESEARCH_MODE_COLORS: [Color; 6] = [
    Color::Rgb(0, 210, 255),
    Color::Rgb(0, 255, 180),
...
const RAMPAGE_MODE_COLORS: [Color; 7] = [
    Color::Rgb(255, 42, 42),
    Color::Rgb(255, 140, 0),
    Color::Rgb(255, 212, 0),
...
fn mode_tab_specs() -> [ModeTabSpec; 5] {
    [
        ModeTabSpec {
            indicator: CollaborationModeIndicator::Ask,
            label: "Ask",
            base_color: Color::Blue,
            palette: &ASK_MODE_COLORS,
        },
        ModeTabSpec {
            indicator: CollaborationModeIndicator::Build,
            label: "Build",
            base_color: Color::Green,
            palette: &BUILD_MODE_COLORS,
        },
        ModeTabSpec {
            indicator: CollaborationModeIndicator::Plan,
            label: "Plan",
            base_color: Color::Magenta,
            palette: &PLAN_MODE_COLORS,
        },
        ModeTabSpec {
            indicator: CollaborationModeIndicator::ReadonlyResearch,
            label: "READONLY RESEARCH",
            base_color: Color::Rgb(0, 210, 255),
            palette: &RESEARCH_MODE_COLORS,
        },
        ModeTabSpec {
            indicator: CollaborationModeIndicator::AbsoluteRampage,
            label: "ABSOLUTE RAMPAGE MODE",
            base_color: Color::Rgb(255, 90, 0),
            palette: &RAMPAGE_MODE_COLORS,
        },
    ]
}

fn mode_tabs_spans(
    active_indicator: CollaborationModeIndicator,
    show_cycle_hint: bool,
) -> Vec<Span<'static>> {
    let mut spans = Vec::new();
    if show_cycle_hint {
        spans.push("Tab".bold());
        spans.push(" changes mode".dim());
        spans.push(" · ".dim());
    }

    let frame = animated_mode_frame();
    let spec = mode_tab_specs()
        .into_iter()
        .find(|spec| spec.indicator == active_indicator)
        .expect("all collaboration mode indicators should have a footer spec");
    ...
    push_mode_label_chars(&mut spans, spec.label, spec.palette, frame);
    spans.push(Span::styled(
        "]",
        Style::default()
            .fg(spec.base_color)
    ...
    match active_indicator {
        CollaborationModeIndicator::ReadonlyResearch => {
            spans.push(" ".into());
            push_meter_spans(&mut spans, &RESEARCH_MODE_COLORS, frame, false);
        }
        CollaborationModeIndicator::AbsoluteRampage => {
            spans.push(" ".into());
            push_meter_spans(&mut spans, &RAMPAGE_MODE_COLORS, frame, true);
        }
        _ => {}
    }
...
#[cfg(not(test))]
fn animated_mode_frame() -> usize {
    SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map(|duration| (duration.as_millis() / 90) as usize)
        .unwrap_or(0)
}
...
fn push_mode_label_chars(
    ...
    for (idx, ch) in label.chars().enumerate() {
        spans.push(Span::styled(
            ch.to_string(),
            Style::default()
                .fg(palette[(idx + frame) % palette.len()])
                .add_modifier(Modifier::BOLD),
        ));
    }
}
...
fn push_meter_spans(
    ...
    const IMPACT_FRAMES: [&str; 6] = ["▁▃▅█", "▃▅█▅", "▅█▅▃", "█▅▃▁", "▅▃▁▃", "▃▁▃▅"];
    const SCAN_FRAMES: [&str; 6] = ["▁▂▃▂", "▂▃▂▁", "▃▂▁▂", "▂▁▂▃", "▁▂▃▅", "▂▃▅▃"];
    let frames = if impact { IMPACT_FRAMES } else { SCAN_FRAMES };
    for (idx, ch) in frames[frame % frames.len()].chars().enumerate() {
        spans.push(Span::styled(
            ch.to_string(),
            Style::default()
                .fg(palette[(idx + frame) % palette.len()])
...
fn left_side_line(
    ...
    if let Some(collaboration_mode_indicator) = collaboration_mode_indicator {
        ...
        for span in collaboration_mode_indicator.styled_spans(state.show_cycle_hint) {
            line.push_span(span);
        }
    }
...
pub(crate) fn single_line_footer_layout(
    ...
    // the right-side context indicator if the "(tab to cycle)" variant
    // can also fit.
    ...
    if show_queue_hint {
        ...
        // outlive "(tab to cycle)" on the left.
...
pub(crate) fn mode_indicator_line(
    ...
    indicator.map(|indicator| Line::from(indicator.styled_spans(show_cycle_hint)))
}
...
pub(crate) fn passive_footer_status_line_with_mode_prefix(
    props: &FooterProps,
    collaboration_mode_indicator: Option<CollaborationModeIndicator>,
    show_cycle_hint: bool,
) -> Option<Line<'static>> {
    let line = passive_footer_status_line(props)?;
    let Some(indicator) = collaboration_mode_indicator else {
        return Some(line);
    };

    Some(prepend_status_segment(
        line,
        indicator.styled_spans(show_cycle_hint),
    ))
}

/// Prefixes the mode indicator without consuming the first configured status
/// item, so items like `model-with-reasoning` stay visible next to `[Build]`.
fn prepend_status_segment(
    line: Line<'static>,
    ...
    spans.push(STATUS_SEGMENT_SEPARATOR.dim());
