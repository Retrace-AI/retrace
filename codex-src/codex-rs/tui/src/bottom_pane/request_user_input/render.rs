    ...
    pub(super) fn render_ui(&self, area: Rect, buf: &mut Buffer) {
        ...
        for (offset, line) in sections.question_lines.iter().enumerate() {
            ...
            let question_line = if answered {
                ...
                // Theme-aware accent (darker/bold on light backgrounds); raw `.cyan()`
                // is nearly invisible on a light terminal.
                Line::from(line.clone()).patch_style(crate::style::accent_style())
            };
