...
pub(crate) struct ChatWidget {
    ...
    live_collab_agent_status: multi_agents::LiveAgentStatus,
    ...
    thinking_display_override: Option<bool>,
    // Whether the working spinner was started by a /model probe (cleared when
    // the probe result arrives, unless a real turn owns the spinner).
    model_probe_spinner_active: bool,
    // The user's chosen permission level, restored whenever leaving a gated
    // collaboration mode (Ask / Readonly Research). Captured at construction
    // and updated only when the user explicitly picks a /permissions preset —
    // never from mode-gate writes or async session echoes, so rapid mode
    // switching cannot corrupt it.
    user_base_permissions: (
        AskForApproval,
        codex_protocol::models::PermissionProfile,
        Option<ActivePermissionProfile>,
    ),
    // Gate change computed during a mode switch, consumed by the very next
    // collaboration-mode override op so mode + permissions land atomically.
    pending_mode_gate_override: Option<(
        AskForApproval,
        codex_protocol::models::PermissionProfile,
        Option<ActivePermissionProfile>,
    )>,
...
impl ChatWidget {
    pub(crate) fn on_collab_agent_activity_started(&mut self, thread_id: ThreadId) {
        if !self.turn_lifecycle.agent_turn_running {
            return;
        }
        let metadata = self.collab_agent_metadata(thread_id);
        if self
            .live_collab_agent_status
            .mark_agent_activity_started(thread_id, metadata)
        {
            self.bottom_pane
                .set_live_agent_status(self.live_collab_agent_status.clone());
        }
    }

    pub(crate) fn on_collab_agent_activity_delta(
        &mut self,
        thread_id: ThreadId,
        item_id: &str,
        delta: &str,
        ...
        if self
            .live_collab_agent_status
            .apply_agent_activity_delta(thread_id, metadata, item_id, delta)
        {
            self.bottom_pane
                .set_live_agent_status(self.live_collab_agent_status.clone());
        }
    ...
    pub(crate) fn set_collab_agent_metadata(
        ...
        if self
            .live_collab_agent_status
            .update_metadata(thread_id, metadata)
        {
            self.bottom_pane
                .set_live_agent_status(self.live_collab_agent_status.clone());
        }
