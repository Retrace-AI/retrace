...
pub(crate) fn load_local_chatgpt_auth(
    ...
    if matches!(auth.auth_mode, Some(AuthMode::ApiKey)) || auth.openai_api_key.is_some() {
        return Err("local auth is not a Retrace login".to_string());
    }

    let tokens = auth
        .tokens
        .ok_or_else(|| "local Retrace auth is missing token data".to_string())?;
    let access_token = tokens.access_token;
    let chatgpt_account_id = tokens
        .account_id
        .or(tokens.id_token.chatgpt_account_id.clone())
        .ok_or_else(|| "local Retrace auth is missing chatgpt account id".to_string())?;
    if let Some(expected_workspaces) = forced_chatgpt_workspace_id
        && !expected_workspaces.contains(&chatgpt_account_id)
    {
        return Err(format!(
            "local Retrace auth must use one of workspace(s) {expected_workspaces:?}, but found {chatgpt_account_id:?}",
        ));
    }
...
mod tests {
    ...
    fn rejects_api_key_auth() {
        ...
        assert_eq!(err, "local auth is not a Retrace login");
    }
