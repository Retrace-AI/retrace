...
impl SlashCommand {
    ...
    pub fn description(self) -> &'static str {
        match self {
            ...
            SlashCommand::Init => "create an AGENTS.md file with instructions for Retrace",
            SlashCommand::Compact => "summarize conversation to prevent hitting the context limit",
            SlashCommand::Review => "review my current changes and find issues",
            SlashCommand::Rename => "rename the current thread",
            SlashCommand::Resume => "resume a saved chat",
            SlashCommand::Archive => "archive this session and exit",
            SlashCommand::Delete => "delete saved conversations (pick one, or \"all\")",
            SlashCommand::Clear => "clear the terminal and start a new chat",
            SlashCommand::Fork => "fork the current chat",
            SlashCommand::App => "continue this session in Retrace Desktop",
            SlashCommand::Quit | SlashCommand::Exit => "exit Retrace",
            SlashCommand::Copy => "copy last response as markdown",
            SlashCommand::Raw => "toggle raw scrollback mode for copy-friendly terminal selection",
            SlashCommand::Diff => "show git diff (including untracked files)",
            SlashCommand::Mention => "mention a file",
            SlashCommand::Skills => "use skills to improve how Retrace performs specific tasks",
            ...
            SlashCommand::AgentCheck => {
                "for weaker models: re-check each answer and auto-retry until fully answered (on|off|status)"
            }
            SlashCommand::Ide => {
                "include current selection, open files, and other context from your IDE"
            }
            SlashCommand::Personality => "choose a communication style for Retrace",
            SlashCommand::Realtime => "toggle realtime voice mode (experimental)",
            SlashCommand::Settings => "configure realtime microphone/speaker",
            SlashCommand::Plan => "switch to Plan mode",
            SlashCommand::Goal => "set or view the goal for a long-running task",
            SlashCommand::Loop => "repeat until stopped: /loop [15s] <prompt>",
            SlashCommand::RalphLoop => {
                "repeat until stopped or a configured iteration limit: /ralphloop <prompt>"
            }
            SlashCommand::Agent | SlashCommand::MultiAgents => "switch the active agent thread",
            SlashCommand::Side | SlashCommand::Btw => {
                "start a side conversation in an ephemeral fork"
            }
            SlashCommand::Permissions => "choose what Retrace is allowed to do",
            ...
            SlashCommand::Logout => "log out of Retrace",
...
mod tests {
    ...
    fn certain_commands_are_available_during_task() {
        ...
        assert!(SlashCommand::RalphLoop.supports_inline_args());
        assert!(SlashCommand::Loop.available_during_task());
        assert!(SlashCommand::RalphLoop.available_during_task());
    }

    #[test]
    fn prompt_loop_command_names_are_stable() {
        assert_eq!(SlashCommand::Loop.command(), "loop");
        assert_eq!(SlashCommand::RalphLoop.command(), "ralphloop");
        assert_eq!(
            SlashCommand::from_str("ralphloop"),
            Ok(SlashCommand::RalphLoop)
        );
        assert_eq!(
