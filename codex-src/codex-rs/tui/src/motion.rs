...
fn animated_activity_indicator(start_time: Option<Instant>) -> Span<'static> {
    ...
    let frames = ["◐", "◓", "◑", "◒"];
    let frame_index = (elapsed.as_millis() / 120) as usize % frames.len();
...
mod tests {
    ...
    #[test]
    fn animated_activity_indicator_uses_circle_spinner_frame() {
        let indicator = activity_indicator(
            Some(Instant::now()),
            MotionMode::Animated,
            ReducedMotionIndicator::StaticBullet,
        )
        .expect("animated indicator should be visible");

        assert!(["◐", "◓", "◑", "◒"].contains(&indicator.content.as_ref()));
    }
