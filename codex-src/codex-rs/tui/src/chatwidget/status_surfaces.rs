...
impl ChatWidget {
    ...
    pub(super) fn status_line_value_for_item(&mut self, item: StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::TotalInputTokens => Some(Self::usage_with_last_display(
                "in",
                self.status_line_total_usage().input_tokens,
                self.status_line_last_usage().input_tokens,
            )),
            StatusLineItem::TotalOutputTokens => Some(Self::usage_with_last_display(
                "out",
                self.status_line_total_usage().output_tokens,
                self.status_line_last_usage().output_tokens,
            )),
            StatusLineItem::TotalCachedTokens => Some(Self::usage_with_last_display(
                "cached",
                self.status_line_total_usage().cached_input_tokens.max(0),
                self.status_line_last_usage().cached_input_tokens.max(0),
            )),
            StatusLineItem::SessionId => self.thread_id.map(|id| id.to_string()),
            StatusLineItem::FastMode => None,
    ...
    pub(super) fn status_surface_preview_value_for_item(
        ...
        let status_line_item = match item {
            ...
            StatusSurfacePreviewItem::TotalCachedTokens => StatusLineItem::TotalCachedTokens,
    ...
    /// Formats a session-total token count with the most recent request's count
    /// in brackets, e.g. `190K in (20K)`. The bracket is omitted until a
    /// request has completed.
    fn usage_with_last_display(label: &str, total: i64, last: i64) -> String {
        if last > 0 {
            format!(
                "{} {label} ({})",
                format_tokens_compact(total),
                format_tokens_compact(last)
            )
        } else {
            format!("{} {label}", format_tokens_compact(total))
        }
    }
