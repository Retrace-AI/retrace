...
impl ChatWidget {
    ...
    pub(super) fn on_collab_agent_tool_call(&mut self, item: ThreadItem) {
        ...
        let active_turn_live_status = self.turn_lifecycle.agent_turn_running;
        if active_turn_live_status {
            let metadata = self.collab_agent_metadata.clone();
            self.live_collab_agent_status.apply_tool_call(
                &item,
                cached_spawn_request.as_ref(),
                |thread_id| metadata.get(&thread_id).cloned().unwrap_or_default(),
            );
            self.bottom_pane
                .set_live_agent_status(self.live_collab_agent_status.clone());
        }

        if multi_agents::should_render_collab_tool_call_in_history(tool, active_turn_live_status) {
