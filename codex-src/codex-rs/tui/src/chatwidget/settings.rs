...
impl ChatWidget {
    ...
    pub(crate) fn set_permission_profile_from_session_snapshot(
        ...
        self.maybe_refresh_user_base_permissions();
        self.refresh_status_surfaces();
        Ok(())
    }
    ...
    /// Refreshes the gated-mode restore point from the current permissions,
    /// unless a gated collaboration mode is active (its narrowed permissions
    /// must never become the restore target).
    fn maybe_refresh_user_base_permissions(&mut self) {
        if Self::mode_gate_preset_id(self.active_mode_kind()).is_none() {
            self.refresh_user_base_permissions();
        }
    }
    ...
    fn apply_thread_settings(&mut self, mut settings: ThreadSettings) {
        ...
        // Track the user's effective permission level as the restore point for
        // gated collaboration modes — but only while no gate is active, so the
        // gate's own (narrower) echoes never become the restore target.
        if Self::mode_gate_preset_id(self.active_mode_kind()).is_none() {
            self.refresh_user_base_permissions();
        }
    ...
    fn collaboration_mode_indicator(&self) -> Option<CollaborationModeIndicator> {
        ...
        match self.active_mode_kind() {
            ...
            ModeKind::Default => Some(CollaborationModeIndicator::Build),
            ModeKind::Plan => Some(CollaborationModeIndicator::Plan),
            ModeKind::ReadonlyResearch => Some(CollaborationModeIndicator::ReadonlyResearch),
            ModeKind::AbsoluteRampage => Some(CollaborationModeIndicator::AbsoluteRampage),
    ...
    /// Refreshes the restore point used when leaving a gated collaboration
    /// mode. Called only from explicit user permission choices.
    pub(crate) fn refresh_user_base_permissions(&mut self) {
        self.user_base_permissions = (
    ...
    /// Applies or restores the OS-sandbox gate when the collaboration mode changes.
    fn apply_mode_permission_gate(&mut self, previous: ModeKind, next: ModeKind) {
        let next_preset_id = Self::mode_gate_preset_id(next);
        let previous_gated = Self::mode_gate_preset_id(previous).is_some();
        match next_preset_id {
            Some(preset_id) => {
            ...
            None => {
                if previous_gated {
                    let (approval, profile, mut active) = self.user_base_permissions.clone();
                    // The server-side update is keyed by the ACTIVE PROFILE ID
                    // (the raw profile is dropped by the op mapper), so a
                    // baseline that came from a legacy --sandbox flag needs the
                    // matching built-in id synthesized or the restore is a no-op.
                    if active.is_none() {
                        active = builtin_approval_presets()
                            .into_iter()
                            .find(|preset| preset.permission_profile == profile)
                            .map(|preset| preset.active_permission_profile.clone());
                    }
                    self.apply_mode_gate_permissions(
                        approval,
                        profile,
    ...
    fn apply_mode_gate_permissions(
        &mut self,
        approval: AskForApproval,
        profile: PermissionProfile,
        active: Option<ActivePermissionProfile>,
        message: String,
    ) {
        // The server-side change rides the same override op as the mode switch
        // (see submit_collaboration_mode_settings_update); sending a separate
        // op here would race its echo against the mode op's echo.
        self.pending_mode_gate_override = Some((approval, profile.clone(), active.clone()));
        self.set_approval_policy(approval);
        if let Err(err) =
            self.set_permission_profile_with_active_profile(profile.clone(), active.clone())
        {
            // Local constraints can reject re-widening (e.g. restoring Full
            // Access after a gated mode narrowed the profile). The session
            // already accepted the override above, so mirror it forcibly.
            tracing::debug!(%err, "mode-gate profile rejected by local constraints; replacing");
            if let Err(err) = self
                .config
                .permissions
                .replace_permission_profile_from_session_snapshot(
                    PermissionProfileSnapshot::from_session_snapshot(profile, active),
                )
            {
                self.add_error_message(format!(
                    "mode-gate: failed to mirror permission profile locally: {err}"
                ));
            } else {
                self.refresh_status_surfaces();
            }
    ...
    pub(crate) fn set_collaboration_mask(&mut self, mut mask: CollaborationModeMask) {
        ...
        if previous_mode != next_mode {
            self.apply_mode_permission_gate(previous_mode, next_mode);
        }
    ...
    fn submit_collaboration_mode_settings_update(&mut self) {
        let pending_gate = self.pending_mode_gate_override.take();
        let Some(thread_id) = self.thread_id else {
            return;
        };
        // Mode and gated permissions travel in ONE override op so their server
        // echo is atomic; separate ops raced and the stale echo won.
        let (approval, profile, active) = match pending_gate {
            Some((approval, profile, active)) => (Some(approval), Some(profile), active),
            None => (None, None, None),
        };
