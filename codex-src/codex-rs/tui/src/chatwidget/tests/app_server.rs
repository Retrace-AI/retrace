...
#[tokio::test]
async fn active_collab_agent_events_render_live_status_not_history() {
    let (mut chat, mut rx, _ops) = make_chatwidget_manual(/*model_override*/ None).await;
    let sender_thread_id = ThreadId::new();
    let spawned_thread_id = ThreadId::new();
    ...
    let prompt = "Connect to the remote server and explore the repo for auth handlers.";
    chat.handle_server_notification(
        ServerNotification::ItemStarted(ItemStartedNotification {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
    ...
    let history = drain_insert_history(&mut rx)
        ...
        .join("\n");
    assert!(
        !history.contains("Spawning")
            && !history.contains("Spawned")
            && !history.contains("Waiting for agents"),
        "active collab lifecycle rows should stay out of transcript history, got {history:?}"
    );

    let bottom = render_bottom_popup(&chat, /*width*/ 100);
    assert!(
        bottom.contains("Waiting for agent") && bottom.contains("Explore the repo"),
        "expected live agent status in bottom pane, got {bottom:?}"
    );
}
