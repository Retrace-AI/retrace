...
async fn plugins_popup_truncates_long_descriptions_in_list_rows() {
    ...
    insta::assert_snapshot!(
        verbose_row,
        @"  [-] Verbose Plugin  Available · Retrace Marketplace · This descri…"
    );
    assert!(
        !popup
...
async fn plugins_popup_openai_curated_tab_omits_marketplace_in_rows() {
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 100);
    assert!(
        popup.contains("Retrace Curated marketplace."),
        "expected Retrace Curated tab header, got:\n{popup}"
    );
    assert!(
        popup.contains("Calendar") && !popup.contains("Repo Plugin"),
        "expected Retrace Curated tab to show only official marketplace plugins, got:\n{popup}"
    );
    assert!(
        !popup.contains("Retrace Marketplace ·"),
...
#[tokio::test]
async fn model_picker_add_custom_opens_model_add_surface() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("test-visible-model")).await;
    chat.thread_id = Some(ThreadId::new());
    ...
    chat.open_model_popup_with_presets(vec![preset("test-visible-model")]);
    while rx.try_recv().is_ok() {}

    // The current model row (`test-visible-model`) auto-focuses via `is_current`,
    // so move up to the first row — "Add custom model" — before selecting it.
    chat.handle_key_event(KeyEvent::from(KeyCode::Up));
    chat.handle_key_event(KeyEvent::from(KeyCode::Enter));

    assert_matches!(rx.try_recv(), Ok(AppEvent::OpenModelAddPicker));
}
