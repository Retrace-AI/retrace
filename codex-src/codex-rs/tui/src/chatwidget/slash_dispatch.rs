...
fn codexos_admin_command(args: Vec<String>) -> CodexOsLocalCommand {
...
async fn run_codexos_local_command_checked(command: CodexOsLocalCommand) -> Result<String, String> {
    let CodexOsLocalCommand {
        label,
        program,
        args,
    ...
    fn apply_model_add(
        ...
        tokio::spawn(async move {
            ...
            tx.send(AppEvent::CodexOsProviderConfigured {
                result,
                success_message: Some(
                    "Model list updated; run /model to switch.".to_string(),
                ),
            });
        });
    }
