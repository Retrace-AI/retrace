...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct CodexOsProviderRow {
    pub(crate) id: String,
...
/// One saved conversation shown in the /delete picker.
#[derive(Debug, Clone)]
pub(crate) struct DeleteConversationRow {
    pub(crate) path: std::path::PathBuf,
    pub(crate) label: String,
}

/// What a /delete action targets.
#[derive(Debug, Clone)]
pub(crate) enum DeleteConversationTarget {
    /// Delete every saved conversation (re-enumerated at delete time).
    All,
    /// Delete a single conversation by its rollout file path.
    One(std::path::PathBuf),
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct CodexOsProviderModelRow {
    pub(crate) id: String,
    ...
    pub(crate) cap_thinking: bool,
    pub(crate) usable: bool,
    pub(crate) thinking_levels: Vec<String>,
    pub(crate) thinking_method: Option<String>,
}

#[derive(Debug, Clone)]
pub(crate) struct CodexOsProviderRemoveSnapshot {
    pub(crate) providers: Vec<CodexOsProviderRow>,
    pub(crate) models: Vec<CodexOsProviderModelRow>,
}

#[derive(Debug, Clone)]
pub(crate) struct CodexOsProviderConfigureResult {
    pub(crate) output: String,
...
pub(crate) enum AppEvent {
    ...
    /// Result of listing configured providers.
    CodexOsProviderListResult(String),

    /// Continue the CodexOpenSource provider wizard after collecting the base URL.
    CodexOsProviderUrlSubmitted {
        base_url: String,
    },

    /// The provider wizard has started probing the remote endpoint.
    CodexOsProviderProbeStarted {
        provider_id: String,
    },

    /// Result of connecting and probing a CodexOpenSource provider.
    CodexOsProviderConnected {
        provider_id: String,
        result: Result<Vec<CodexOsProviderModelRow>, String>,
    },

    /// Models selected from a newly connected provider.
    CodexOsProviderModelsSelected {
        provider_id: String,
        rows: Vec<CodexOsProviderModelRow>,
        model_ids: Vec<String>,
    },

    /// Result of enabling selected provider models and applying settings.
    CodexOsProviderConfigured {
        result: Result<CodexOsProviderConfigureResult, String>,
    ...
    /// Result of loading the model list for the `/model probe` picker.
    CodexOsModelProbeListLoaded {
        result: Result<Vec<CodexOsProviderModelRow>, String>,
    },
    ...
    /// `/model`: after a model and its variant are chosen, ask for that model's
    /// context window. Limits are per-model — each model has its own real
    /// window — so this carries the single selected model, never a set.
    ...
    /// still needs an exact number typed in.
    ...
    /// The output limit chosen for `model`; writes both limits for that model
    /// and then applies the model/effort selection.
    ModelOutputSelected {
        model: String,
        effort: Option<ReasoningEffort>,
        context_window: i64,
    ...
    DeleteConversationsListLoaded {
        rows: Vec<DeleteConversationRow>,
    },

    /// Delete a saved conversation (or all). `confirmed=false` shows a confirm
    /// prompt first; `confirmed=true` performs the deletion.
    DeleteConversations {
        target: DeleteConversationTarget,
        confirmed: bool,
    },

    /// Result of a conversation deletion (count removed, or an error).
    DeleteConversationsDone {
        removed: Result<usize, String>,
    },

    /// The user explicitly chose a permission preset; refresh the restore
    /// point used when leaving gated collaboration modes.
    MarkUserPermissionBaseline,

    /// A model was chosen in the `/model probe` picker (or via `/model probe <id>`).
    CodexOsReprobeModel {
        model_id: String,
    },

    /// A provider was selected for removal.
    CodexOsProviderRemoveProviderSelected {
        snapshot: CodexOsProviderRemoveSnapshot,
        provider_id: String,
    },

    /// Open the model picker for removing selected provider models.
    CodexOsProviderRemoveModelsRequested {
        snapshot: CodexOsProviderRemoveSnapshot,
        provider_id: String,
    },

    /// Remove an entire provider and all of its models.
    CodexOsProviderRemoveEntire {
        provider_id: String,
    },

    /// Remove selected models from a provider.
    CodexOsProviderRemoveModelsSelected {
        provider_id: String,
        model_ids: Vec<String>,
    },
