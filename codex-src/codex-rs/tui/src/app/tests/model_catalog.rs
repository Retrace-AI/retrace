...
#[tokio::test]
async fn prepare_startup_tooltip_override_prompts_to_add_models_when_none_configured() {
    ...
    assert!(
        tooltip.contains("/model") && tooltip.contains("Add custom model"),
        "unexpected no-models tooltip: {tooltip}"
    );
}
