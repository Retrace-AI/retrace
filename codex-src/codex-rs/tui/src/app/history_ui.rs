...
const DESKTOP_THREAD_OPENED_MESSAGE: &str = "Opened this session in Retrace Desktop.";
...
fn desktop_thread_open_error_message(err: &str) -> String {
    format!(
        "Failed to open this session in Retrace Desktop: {err}. Install or launch Retrace Desktop and try again."
    )
}
...
fn open_desktop_thread_url(url: &str) -> Result<(), String> {
    ...
    let output = std::process::Command::new("powershell.exe")
        ...
        .arg(&script)
        .output()
        .map_err(|err| format!("failed to launch Retrace Desktop through PowerShell: {err}"))?;
    ...
    if stderr.is_empty() {
        Err(format!(
            "failed to launch Retrace Desktop through PowerShell with {}",
            output.status
        ))
    } else {
        Err(stderr)
...
$installLocation = (Get-AppxPackage -Name Retrace.Retrace -ErrorAction SilentlyContinue).InstallLocation
if ([string]::IsNullOrWhiteSpace($installLocation)) {{
    Write-Error 'Retrace Desktop package is not installed'
    exit 1
}}

$appDir = Join-Path $installLocation 'app'
$exe = Join-Path $appDir 'Retrace.exe'
...
fn open_desktop_thread_url(_url: &str) -> Result<(), String> {
    Err("Retrace Desktop is only available on macOS and Windows".to_string())
}
