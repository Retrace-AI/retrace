...
impl App {
    pub(super) async fn handle_event(
        ...
        match event {
            ...
            AppEvent::CodexOsProviderListResult(text) => {
                let lines: Vec<ratatui::text::Line<'static>> = if text.trim().is_empty() {
                    vec!["/provider list produced no output.".italic().into()]
                } else {
                    text.lines().map(ansi_escape_line).collect()
                };
                self.chat_widget.add_plain_history_lines(lines);
            ...
            AppEvent::CodexOsProviderUrlSubmitted { base_url } => {
                self.chat_widget.open_codexos_provider_key_prompt(base_url);
                tui.frame_requester().schedule_frame();
            }
            AppEvent::CodexOsProviderProbeStarted { provider_id } => {
                self.chat_widget
                    .on_codexos_provider_probe_started(provider_id);
                tui.frame_requester().schedule_frame();
            }
            AppEvent::CodexOsProviderConnected {
                provider_id,
                result,
            } => {
                self.chat_widget
                    .on_codexos_provider_connected(provider_id, result);
                tui.frame_requester().schedule_frame();
            }
            AppEvent::CodexOsProviderModelsSelected {
                provider_id,
                rows,
                model_ids,
            } => {
                self.chat_widget
                    .on_codexos_provider_models_selected(provider_id, rows, model_ids);
                tui.frame_requester().schedule_frame();
            }
            AppEvent::CodexOsProviderConfigured {
                result,
                success_message,
            } => {
            ...
            AppEvent::CodexOsProviderRemoveSnapshotLoaded {
                ...
                self.chat_widget
                    .on_codexos_provider_remove_snapshot_loaded(provider_id, result);
                tui.frame_requester().schedule_frame();
            }
            AppEvent::CodexOsModelProbeListLoaded { result } => {
                self.chat_widget.on_codexos_model_probe_list_loaded(result);
                tui.frame_requester().schedule_frame();
            }
            AppEvent::CodexOsReprobeModel { model_id } => {
            ...
            AppEvent::CodexOsModelAddSelectionConfirmed { rows, model_ids } => {
                self.chat_widget
            ...
            AppEvent::ModelContextSelected {
                ...
                self.chat_widget
                    .on_model_context_selected(model, effort, context_window);
                tui.frame_requester().schedule_frame();
            }
            AppEvent::ModelOutputSelected {
                model,
                effort,
                context_window,
                output_tokens,
            } => {
                self.chat_widget.on_model_output_selected(
                    model,
                    effort,
                    context_window,
                    output_tokens,
            ...
            AppEvent::DeleteConversationsListLoaded { rows } => {
                self.chat_widget.on_delete_conversations_list_loaded(rows);
                tui.frame_requester().schedule_frame();
            }
            AppEvent::DeleteConversations { target, confirmed } => {
                self.chat_widget.on_delete_conversations(target, confirmed);
                tui.frame_requester().schedule_frame();
            }
            AppEvent::DeleteConversationsDone { removed } => {
                self.chat_widget.on_delete_conversations_done(removed);
                tui.frame_requester().schedule_frame();
            }
            AppEvent::MarkUserPermissionBaseline => {
                self.chat_widget.refresh_user_base_permissions();
            }
            AppEvent::CodexOsProviderRemoveProviderSelected {
                snapshot,
                provider_id,
            } => {
                self.chat_widget
                    .open_codexos_provider_remove_action_menu(snapshot, provider_id);
                tui.frame_requester().schedule_frame();
            }
            AppEvent::CodexOsProviderRemoveModelsRequested {
                snapshot,
                provider_id,
            } => {
                self.chat_widget
                    .open_codexos_provider_remove_models_picker(snapshot, provider_id);
                tui.frame_requester().schedule_frame();
            }
            AppEvent::CodexOsProviderRemoveEntire { provider_id } => {
                self.chat_widget
                    .on_codexos_provider_remove_entire(provider_id);
                tui.frame_requester().schedule_frame();
            }
            AppEvent::CodexOsProviderRemoveModelsSelected {
                provider_id,
                model_ids,
            } => {
                self.chat_widget
                    .on_codexos_provider_remove_models_selected(provider_id, model_ids);
                tui.frame_requester().schedule_frame();
            }
                        ...
                        Ok(_) => {
                                ...
                                self.chat_widget.world_writable_warning_details()
                            {
                                ...
                                self.chat_widget.add_plain_history_lines(vec![
                                    ...
                                    Line::from(vec![
                                        "  ".into(),
                                        "Retrace can now safely edit files and execute commands in your computer"
                                            .dark_gray(),
                                    ]),
                                ]);
                            } else {
