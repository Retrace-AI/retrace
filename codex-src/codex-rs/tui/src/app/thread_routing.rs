...
impl App {
    ...
    pub(super) async fn enqueue_thread_notification(
        ...
        let inferred_session = self
            ...
            .await;
        self.mirror_collab_agent_activity_notification(thread_id, &notification);
        let (sender, store) = {
    ...
    fn mirror_collab_agent_activity_notification(
        &mut self,
        thread_id: ThreadId,
        notification: &ServerNotification,
    ) {
        ...
        match notification {
            ServerNotification::TurnStarted(_) => {
                self.chat_widget.on_collab_agent_activity_started(thread_id);
            }
            ServerNotification::AgentMessageDelta(notification) => {
                self.chat_widget.on_collab_agent_activity_delta(
                    thread_id,
