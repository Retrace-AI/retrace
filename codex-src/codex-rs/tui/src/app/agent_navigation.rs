...
impl AgentNavigationState {
    ...
    pub(crate) fn active_agent_label(
        ...
        match self.team_status_label(primary_thread_id) {
            Some(team_status) => Some(format!("{current} · {team_status}")),
            None => Some(current),
        }
    }

    /// Summarizes visible worker activity for orchestrated modes.
    ///
    /// The picker cache knows when a child thread is first seen and when it closes. That is enough
    /// for a compact live footer signal without polling the backend for per-frame worker state.
    pub(crate) fn team_status_label(&self, primary_thread_id: Option<ThreadId>) -> Option<String> {
        let workers = self
            .ordered_threads()
            .into_iter()
        ...
        let done = workers.iter().filter(|entry| entry.is_closed).count();
        let total = workers.len();
        let working = total.saturating_sub(done);
        Some(format!(
            "Agents - {working} working · {done} done · {total} total"
        ))
    }
...
mod tests {
    ...
    fn active_agent_label_tracks_current_thread() {
        ...
        assert_eq!(
            ...
            Some("Robie [explorer] · Agents - 2 working · 0 done · 2 total".to_string())
        );
        assert_eq!(
            state.active_agent_label(Some(main_thread_id), Some(main_thread_id)),
            Some("Main [default] · Agents - 2 working · 0 done · 2 total".to_string())
        );

        state.mark_closed(second_agent_id);
        assert_eq!(
            state.team_status_label(Some(main_thread_id)),
            Some("Agents - 1 working · 1 done · 2 total".to_string())
        );
    }
}
