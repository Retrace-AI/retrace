...
pub(super) async fn prepare_startup_tooltip_override(
    ...
    // Highest priority: if no models are usable yet, always steer the user to
    // /model to connect a provider (URL + API key). This overrides the random
    // pool and even the first-run suppression, because an empty catalog is the
    // one situation where a generic tip is useless.
    if available_models.is_empty() {
        return Some(
            "No models configured yet — run /model, choose \"Add custom model\", and enter your provider's URL and API key to get started.".to_string(),
        );
    }
