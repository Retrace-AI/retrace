...
///
/// The probe returns as soon as every queried response has arrived, so responsive terminals never
/// pay this in full. It only bounds terminals that stay silent — and a miss is costly: the palette
/// caches "no default colors" for the whole session and the UI falls back to dark-theme colors,
/// which are invisible on light backgrounds. 100ms proved too tight for Terminal.app under load.
