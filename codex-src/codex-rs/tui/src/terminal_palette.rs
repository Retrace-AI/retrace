...
pub fn default_colors() -> Option<DefaultColors> {
    if let Some(colors) = override_colors_from_env() {
        return Some(colors);
    }
    imp::default_colors().or_else(colors_from_colorfgbg)
}

/// Explicit user override for terminals where OSC 10/11 detection is unreliable.
///
/// `CODEX_TUI_BG` accepts `light`, `dark`, or a `#rrggbb` background color. When set it wins over
/// live detection, so a slow or failed startup probe can never flip the UI to the wrong theme.
fn override_colors_from_env() -> Option<DefaultColors> {
    static CACHE: std::sync::OnceLock<Option<DefaultColors>> = std::sync::OnceLock::new();
    *CACHE.get_or_init(|| {
        std::env::var("CODEX_TUI_BG")
            .ok()
            .and_then(|value| parse_bg_override(&value))
    })
}

fn parse_bg_override(value: &str) -> Option<DefaultColors> {
    ...
    let bg = (r, g, b);
    let fg = if crate::color::is_light(bg) {
        (0, 0, 0)
    } else {
        (255, 255, 255)
    };
...
/// Fallback when OSC 10/11 detection fails: some terminals advertise their palette through the
/// legacy `COLORFGBG` variable (`<fg>;<bg>` or `<fg>;default;<bg>`, ANSI indices 0-15). Rough, but
/// far better than silently assuming a dark background on a light terminal.
...
fn parse_colorfgbg(value: &str) -> Option<DefaultColors> {
    ...
    let bg = parts.next_back()?.trim().parse::<usize>().ok()?;
    if fg > 15 || bg > 15 {
        return None;
    }
    Some(DefaultColors {
...
mod imp {
    ...
    pub(super) fn requery_default_colors() {
        if let Ok(mut cache) = default_colors_cache().lock() {
            // A missed startup detection (e.g. Terminal.app answering OSC 10/11 slower than the
            // probe deadline) is worth a few recovery attempts here, otherwise the session stays
            // dark-themed forever. But crossterm's color query blocks up to 2s per slot when the
            // terminal never answers, so cap the attempts instead of stalling every focus gain.
            if cache.attempted && cache.value.is_none() {
                ...
                if RECOVERY_ATTEMPTS.load(Ordering::Relaxed) >= MAX_RECOVERY_ATTEMPTS {
                    return;
                }
            ...
            let bg = query_background_color()
                ...
                .flatten()
                .and_then(color_to_tuple);
            // Only replace the cache when the requery actually succeeded. A focus-triggered
            // requery frequently fails because crossterm's active event stream consumes the
            // OSC 10/11 responses; if we clobbered a previously-good detection with `None`,
            // the whole UI would fall back to a dark-background assumption and light-terminal
            // text would wash out to invisible. Keep the last good value instead.
            if let Some(colors) = fg.zip(bg).map(|(fg, bg)| DefaultColors { fg, bg }) {
...
mod tests {
    ...
    #[test]
    fn parses_bg_override_keywords_and_hex() {
        let light = parse_bg_override("light").unwrap();
        assert_eq!(light.bg, (255, 255, 255));
        assert_eq!(light.fg, (0, 0, 0));

        let dark = parse_bg_override("DARK").unwrap();
        assert_eq!(dark.bg, (0, 0, 0));
        assert_eq!(dark.fg, (255, 255, 255));

        let hex = parse_bg_override("#ffeedd").unwrap();
        assert_eq!(hex.bg, (0xff, 0xee, 0xdd));
        assert_eq!(hex.fg, (0, 0, 0));

        assert!(parse_bg_override("").is_none());
        assert!(parse_bg_override("#12345").is_none());
        assert!(parse_bg_override("bogus").is_none());
    }

    #[test]
    fn parses_colorfgbg_two_and_three_part_forms() {
        let two = parse_colorfgbg("0;15").unwrap();
        assert_eq!(two.fg, XTERM_COLORS[0]);
        assert_eq!(two.bg, XTERM_COLORS[15]);

        let three = parse_colorfgbg("15;default;0").unwrap();
        assert_eq!(three.fg, XTERM_COLORS[15]);
        assert_eq!(three.bg, XTERM_COLORS[0]);

        assert!(parse_colorfgbg("default;default").is_none());
        assert!(parse_colorfgbg("0;99").is_none());
        assert!(parse_colorfgbg("").is_none());
    }
