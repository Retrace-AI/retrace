...
async fn status_model_provider_uses_bedrock_runtime_base_url_and_gates_usage_link() {
    ...
    assert!(
        ...
        "expected /status to hide Retrace usage link for Bedrock, got: {rendered}"
    );

    config.model_provider_id = "openai-proxy".to_string();
    config.model_provider = ModelProviderInfo {
        name: "Retrace Proxy".to_string(),
    ...
    assert!(
        rendered.contains("https://chatgpt.com/codex/settings/usage"),
        "expected /status to show Retrace usage link for Retrace-auth proxy, got: {rendered}"
    );
