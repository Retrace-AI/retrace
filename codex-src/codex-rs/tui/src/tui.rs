...
mod tests {
    ...
    use super::bottom_aligned_inline_viewport_area;
    ...
    #[test]
    fn inline_viewport_bottom_aligns_when_terminal_height_grows() {
        let current = Rect::new(
            /*x*/ 0, /*y*/ 30, /*width*/ 80, /*height*/ 4,
        );
        let screen = ratatui::layout::Size::new(/*width*/ 158, /*height*/ 61);
        let area = bottom_aligned_inline_viewport_area(current, screen, /*height*/ 5);

        assert_eq!(area, Rect::new(0, 56, 158, 5));
    }

    #[test]
    fn inline_viewport_uses_full_width_after_terminal_resize() {
        let current = Rect::new(
            /*x*/ 0, /*y*/ 30, /*width*/ 80, /*height*/ 4,
        );
        let screen = ratatui::layout::Size::new(/*width*/ 158, /*height*/ 61);
        let area = bottom_aligned_inline_viewport_area(current, screen, /*height*/ 80);

        assert_eq!(area, Rect::new(0, 0, 158, 61));
    }
}
...
fn bottom_aligned_inline_viewport_area(mut area: Rect, screen_size: Size, height: u16) -> Rect {
...
impl Tui {
    ...
    fn update_inline_viewport_for_resize_reflow(
        ...
        // When the bottom pane grows in place (same terminal size), the taller bottom-aligned
        // viewport absorbs the rows immediately above it — which are the most recently inserted
        // scrollback rows (e.g. the user message that was just submitted). Scroll the region
        // above the viewport up to make room first, like the legacy draw path does; otherwise
        // the clear below erases those rows and resize reflow never rebuilds them, because it
        // only runs on terminal size changes.
        let terminal_height_shrank = size.height < terminal.last_known_screen_size.height;
        let mut area_with_old_y = previous_area;
        area_with_old_y.height = height.min(size.height);
        area_with_old_y.width = size.width;
        if area_with_old_y.bottom() > size.height && !terminal_height_shrank {
            terminal.backend_mut().scroll_region_up(
                0..area_with_old_y.top(),
                area_with_old_y.bottom() - size.height,
            )?;
        }

        let area = bottom_aligned_inline_viewport_area(terminal.viewport_area, size, height);
    ...
    pub fn draw(
        ...
        stdout().sync_update(|_| {
            ...
            let mut area_with_old_y = terminal.viewport_area;
            area_with_old_y.height = height.min(size.height);
            area_with_old_y.width = size.width;
            // If the viewport has expanded, scroll everything else up to make room.
            if area_with_old_y.bottom() > size.height {
                terminal.backend_mut().scroll_region_up(
                    0..area_with_old_y.top(),
                    area_with_old_y.bottom() - size.height,
                )?;
