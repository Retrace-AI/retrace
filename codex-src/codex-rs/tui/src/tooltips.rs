...
const APP_TOOLTIP: &str = "Use /model to switch models and reasoning effort at any time.";
const FAST_TOOLTIP: &str =
    "*New* Use **/model add** to enable more models from your connected providers.";
const OTHER_TOOLTIP: &str = "*New* /model probe live-detects a model's thinking, effort levels, cache, and streaming.";
const OTHER_TOOLTIP_NON_MAC: &str = "*New* Run /model, choose \"Add custom model\", and enter a provider URL and API key.";
const FREE_GO_TOOLTIP: &str =
    "*New* Add any OpenAI-compatible provider with /model → \"Add custom model\".";
...
pub(crate) mod announcement {
    ...
    ///
    /// Retrace does not fetch remote announcements (the upstream feed is
    /// OpenAI's), so this is a no-op: the OnceLock stays unset and
    /// `fetch_announcement_tip` always returns None.
    pub(crate) fn prewarm() {
        let _ = init_announcement_tip_in_thread; // referenced to avoid dead-code warnings
    }
...
from_date = "2000-01-01"
    ...
    fn announcement_tip_toml_parse_comments() {
        let toml = r#"
# Example announcement tips for Retrace TUI.
