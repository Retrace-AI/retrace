...
5. Spawn focused read-only research/review/verifier workers with `rampage_spawn` when independent research exists.
...
4. Spawn focused read-only workers with `rampage_spawn`.
5. Continue useful non-overlapping local read-only investigation while workers run.
...
11. If the verifier fails, write missing evidence or blocked access to the Questboard and continue. If failures reach the configured limit, the mission is set to `blocked`; ask the user how to proceed with `request_user_input`.
...
4. If `pass_percentage >= verifier_pass_threshold`, the verifier passes and completion is unlocked. Otherwise it counts as a failure; write the missing evidence to the Questboard and continue.
...
The selected support agents are MONITORS, not one-shot startup advisories. Re-invoke them each mission round (spawn a fresh `rampage_spawn` task) with a snapshot of the current task states and Questboard, so they can observe how the real read-only workers are actually progressing. They cannot see live state on their own, so you must feed them the current snapshot each round.
...
- If it is a suggestion, direction, or new constraint, treat it as authoritative: record it on the Questboard (as a decision), fold it into the relevant task by re-tasking the affected read-only worker with `rampage_spawn`, and adjust the question/success criteria if needed.
