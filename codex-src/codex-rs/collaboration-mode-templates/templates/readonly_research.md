...
Advisory Agent (recommended; spawn with `rampage_spawn` role set to exactly `advisory` — a role like "advisory review" is treated as an ordinary worker, not the advisor):
