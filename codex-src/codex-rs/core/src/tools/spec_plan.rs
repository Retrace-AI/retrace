...
use crate::tools::handlers::rampage::RampageToolOptions;
use crate::tools::handlers::rampage::tools_enabled_for_turn as rampage_tools_enabled_for_turn;
...
fn add_rampage_tools(context: &CoreToolPlanContext<'_>, planned_tools: &mut PlannedTools) {
    let turn_context = context.turn_context;
    if !rampage_tools_enabled_for_turn(
        turn_context.collaboration_mode.mode,
        &turn_context.session_source,
    ) {
        return;
    ...
    let options = RampageToolOptions::new(turn_context.collaboration_mode.mode);
    planned_tools.add(RampageControlHandler::new(options));
    planned_tools.add(RampageBoardHandler::new(options));
    planned_tools.add(RampageCompactHandler::new(options));
    if multi_agent_v2_enabled(turn_context) && collab_tools_enabled(turn_context) {
        planned_tools.add(RampageSpawnHandler::new(options));
    }
}
