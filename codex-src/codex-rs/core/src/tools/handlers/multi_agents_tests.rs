...
#[test]
fn multi_agent_v2_interrupt_agent_accepts_task_name_target() {
    run_large_stack_async_test(
        "multi_agent_v2_interrupt_agent_accepts_task_name_target",
        || async {
            let (mut session, mut turn) = make_session_and_context().await;
            let manager = thread_manager();
            let root = manager
            ...
            timeout(Duration::from_secs(5), async {
                loop {
                    ...
                    if ops
                        .iter()
                        .any(|(thread_id, op)| *thread_id == child_id && matches!(op, Op::Shutdown))
                    {
                        break;
                    }
            ...
            .await
            .expect("interrupt_agent should shut down child agents");
        },
    );
}
