...
fn request_user_input_tool_description_mentions_available_modes() {
    assert_eq!(
        ...
        "Request user input for one to three short questions and wait for the response. This tool is only available in modes: Plan, Readonly Research, Absolute Rampage.".to_string()
    );
    assert_eq!(
        request_user_input_tool_description(&default_mode_enabled_available_modes()),
        "Request user input for one to three short questions and wait for the response. This tool is only available in modes: Build, Plan, Readonly Research, Absolute Rampage.".to_string()
    );
}
