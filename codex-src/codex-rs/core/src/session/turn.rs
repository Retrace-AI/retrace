                ...
                if !needs_follow_up {
                    ...
                    if let Some(hook_prompt_message) =
                        build_rampage_active_mission_continuation_message(
                            ...
                            turn_context.as_ref(),
                            last_agent_message.as_deref(),
                            rampage_active_mission_forced_continuations,
                        )
                        .await
                    {
                        sess.record_conversation_items(
                        ...
                        .await;
                        rampage_active_mission_forced_continuations += 1;
                        continue;
                    }
                    if let Some(hook_prompt_message) = build_mid_task_status_continuation_message(
                        turn_context.as_ref(),
                        last_agent_message.as_deref(),
                        mid_task_status_forced_continuations,
                    ) {
                        sess.record_conversation_items(
                            &turn_context,
                            std::slice::from_ref(&hook_prompt_message),
                        ...
                        .await;
                        mid_task_status_forced_continuations += 1;
                        continue;
                    }
...
impl RampageStartupGuard {
    ...
    fn record_blocked_tool_call(&mut self) {
        self.blocked_tool_calls += 1;
    }

    fn blocked_tool_message(&self, tool_name: &ToolCallIdentity) -> String {
        let mode_name = self
            .mode
        ...
        if tool_name.name == "spawn_agent" {
            return format!(
                "{mode_name} blocks raw `{}`. Use `rampage_spawn` so Mission Control creates a durable task row, injects mission and Questboard context, and routes worker results back through the Questboard.",
                tool_name.display()
            );
        }
    ...
    fn build_completion_blocker_message(
        ...
        let previous = last_agent_message
            ...
            .unwrap_or_default();
        let repeated = if self.completion_blocks >= 3 {
            "\n\nThis startup gate has been missed repeatedly. Do not explain or continue normal work. Your next output must be `request_user_input` for the support-agent choice or `rampage_control action=start` if the answer is already available."
        } else {
            ""
        };
        let missing = self.missing_startup_steps().join(", ");
...
struct ToolCallIdentity {
    ...
    name: String,
    call_id: String,
    output_kind: ToolCallOutputKind,
}
...
enum ToolCallOutputKind {
    Function,
    Custom,
    ToolSearch,
}

fn rampage_runtime_enforcement_required(mode: ModeKind, input: &[TurnInput]) -> bool {
    if !matches!(mode, ModeKind::AbsoluteRampage | ModeKind::ReadonlyResearch) {
        return false;
    }
    input.iter().any(turn_input_is_nontrivial_user_task)
}

fn turn_input_is_nontrivial_user_task(input: &TurnInput) -> bool {
...
async fn rampage_runtime_tool_block_message(
    ...
    {
        Ok(Some(status)) => {
            let missing_names = status
                .missing_agents
                .iter()
                .map(|agent| agent.display_name)
                .collect::<Vec<_>>()
                .join(", ");
            let spawn_plan = status
                .missing_agents
                .iter()
                .map(|agent| {
                    format!(
                        "- rampage_spawn task_name=\"{}\" role=\"{}\" kind=\"{}\" title=\"{}\" instructions=\"{}\"",
                        agent.task_name,
                        agent.role,
                        agent.kind,
                        agent.title,
...
fn tool_call_identity(item: &ResponseItem) -> Option<ToolCallIdentity> {
    match item {
        ...
        ResponseItem::CustomToolCall { name, call_id, .. } => Some(ToolCallIdentity {
            namespace: None,
            name: name.clone(),
            call_id: call_id.clone(),
            output_kind: ToolCallOutputKind::Custom,
        }),
        ResponseItem::ToolSearchCall {
            call_id: Some(call_id),
            execution,
            ..
        } if execution == "client" => Some(ToolCallIdentity {
            namespace: None,
            name: "tool_search".to_string(),
            call_id: call_id.clone(),
            output_kind: ToolCallOutputKind::ToolSearch,
        }),
        _ => None,
    }
}

fn blocked_tool_output_item(tool_name: &ToolCallIdentity, message: String) -> ResponseItem {
    let output = FunctionCallOutputPayload {
        body: FunctionCallOutputBody::Text(message),
        success: Some(false),
    };
    match tool_name.output_kind {
        ToolCallOutputKind::Function => ResponseItem::FunctionCallOutput {
            call_id: tool_name.call_id.clone(),
            output,
        },
        ToolCallOutputKind::Custom => ResponseItem::CustomToolCallOutput {
            call_id: tool_name.call_id.clone(),
            name: Some(tool_name.name.clone()),
            output,
        },
        ToolCallOutputKind::ToolSearch => ResponseItem::ToolSearchOutput {
            call_id: Some(tool_name.call_id.clone()),
...
async fn build_active_exec_continuation_message(
    sess: &Arc<Session>,
    turn_context: &TurnContext,
    last_agent_message: Option<&str>,
) -> Option<ResponseItem> {
    ...
    if !sess
        ...
        .has_live_processes_for_session(sess)
        .await
    {
        return None;
    }
    let last_agent_message = last_agent_message?.trim();
    if last_agent_message.is_empty() || message_has_verified_final_marker(last_agent_message) {
        return None;
    }
    let mode_name = turn_context.collaboration_mode.mode.display_name();
    let prompt = format!(
        "{mode_name} mode cannot complete while a unified exec session is still active.\n\nPrevious visible message:\n{last_agent_message}\n\nContinue now by polling the running session with `write_stdin`, inspecting the active process, or reporting the real blocker. Do not provide a final answer until the active session has finished or you have intentionally stopped it."
    );
    build_hook_prompt_message(&[HookPromptFragment::from_single_hook(
        prompt,
        "active-exec-continue",
    )])
}

async fn build_rampage_active_mission_continuation_message(
    sess: &Session,
    turn_context: &TurnContext,
    last_agent_message: Option<&str>,
    ...
    if !matches!(
        turn_context.collaboration_mode.mode,
        ModeKind::ReadonlyResearch | ModeKind::AbsoluteRampage
    ) {
        return None;
    }
    ...
    .await
    {
        ...
        Err(err) => {
            ...
            let previous = last_agent_message
                ...
                .unwrap_or_default();
            let prompt = format!(
                "{mode_name} mode could not read the durable Rampage mission state: {err}.{previous}\n\nDo not provide a final answer. Call `rampage_control` with action=status to recover the Mission Control state, then continue or report the smallest concrete blocker."
            );
            return build_hook_prompt_message(&[HookPromptFragment::from_single_hook(
                prompt,
    ...
    let previous = last_agent_message
        ...
        .map(|message| format!("\n\nPrevious visible message:\n{message}"))
        .unwrap_or_default();
    let repeated_guidance = if forced_continuations >= 3 {
        "\n\nThis active mission has already blocked completion repeatedly. Do not answer with more status prose. Use a concrete tool call now."
    } else {
        ""
    };
    ...
    let prompt = format!(
        ...
        board_items = status.board_items,
...
fn build_mid_task_status_continuation_message(
    ...
    let last_agent_message = last_agent_message?.trim();
    if !message_needs_mid_task_continuation(last_agent_message) {
        return None;
    }

    let mode_name = turn_context.collaboration_mode.mode.display_name();
    let repeated_guidance = if forced_continuations >= 6 {
        "\n\nThis has happened repeatedly in this turn. Do not answer with another progress sentence. Use a concrete tool call now, wait on active work, inspect the next source of truth, or state the real blocker."
    } else {
        ""
    };
    let prompt = format!(
        "{mode_name} mode stopped after a mid-task status message instead of taking the next action.\n\nPrevious visible message:\n{last_agent_message}\n\nContinue now. Do not restate intent. Execute the next concrete tool/action, wait for active work, or provide a verified final answer only if the user's goal is actually complete. If blocked, ask for the smallest missing access or approval needed to continue.{repeated_guidance}"
    );
    build_hook_prompt_message(&[HookPromptFragment::from_single_hook(
        prompt,
        "mid-task-status-continue",
    )])
}

fn message_needs_mid_task_continuation(message: &str) -> bool {
    ...
    if lower.contains("let me know")
        ...
        || lower.contains("verified final")
    {
        return false;
    }

    if message_has_verified_final_marker(trimmed) {
        return false;
    }
    ...
    let has_intent_marker = intent_markers.iter().any(|marker| lower.contains(marker));
...
fn message_has_verified_final_marker(message: &str) -> bool {
    let lower = message.trim().to_ascii_lowercase();
    let has_final_marker = lower.starts_with("done")
...
async fn run_sampling_request(
    ...
    input: Vec<ResponseItem>,
    rampage_startup_guard: &mut RampageStartupGuard,
    cancellation_token: CancellationToken,
                ...
                if let Some(tool_name) = tool_call_identity(&item) {
                    if rampage_startup_guard.should_block_tool_call(&tool_name) {
                        ...
                        record_completed_response_item(sess.as_ref(), turn_context.as_ref(), &item)
                            .await;
                        let output_item = blocked_tool_output_item(&tool_name, message);
                        sess.record_conversation_items(
                            &turn_context,
