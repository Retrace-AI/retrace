...
impl Session {
    ...
    pub(crate) async fn get_base_instructions(&self) -> BaseInstructions {
        let state = self.state.lock().await;
        // Substitute the configurable product/company brand into the base prompt.
        // Defaults to "Retrace"; set RETRACE_AGENT_NAME to rebrand (e.g. per company).
        let brand = std::env::var("RETRACE_AGENT_NAME")
            .ok()
