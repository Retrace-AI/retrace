...
fn run_large_stack_async_test<F, Fut>(name: &str, test: F)
where
...
fn shutdown_agent_tree_closes_live_descendants() {
    run_large_stack_async_test("shutdown_agent_tree_closes_live_descendants", || async {
        let harness = AgentControlHarness::new().await;
        let (parent_thread_id, _parent_thread) = harness.start_thread().await;
...
#[test]
fn shutdown_live_agent_descendants_closes_live_subtree() {
    let handle = std::thread::Builder::new()
        .name("shutdown_live_agent_descendants_closes_live_subtree".to_string())
        .stack_size(32 * 1024 * 1024)
        .spawn(|| {
            let runtime = tokio::runtime::Builder::new_multi_thread()
                .worker_threads(2)
            ...
            runtime.block_on(async {
                ...
                let mut shutdown_agents = harness
                    .control
                    .shutdown_live_agent_descendants(parent_thread_id)
                    .await
                    .expect("descendant shutdown should succeed");
                shutdown_agents.sort_by_key(std::string::ToString::to_string);
                let mut expected_shutdown_agents = vec![child_thread_id, grandchild_thread_id];
                expected_shutdown_agents.sort_by_key(std::string::ToString::to_string);
                assert_eq!(shutdown_agents, expected_shutdown_agents);

                let ops = harness.manager.captured_ops();
                assert!(!ops.iter().any(|(thread_id, op)| {
                    *thread_id == parent_thread_id && matches!(op, Op::Interrupt)
                }));
                let mut shutdown_ids = ops
                    .iter()
                    .filter_map(|(thread_id, op)| matches!(op, Op::Shutdown).then_some(*thread_id))
                    .collect::<Vec<_>>();
                shutdown_ids.sort_by_key(std::string::ToString::to_string);
                assert_eq!(shutdown_ids, expected_shutdown_agents);
            });
        })
        .expect("test thread should spawn");
