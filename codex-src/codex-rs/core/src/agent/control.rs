...
impl AgentControl {
    ...
    /// Shut down all live thread-spawn descendants of `parent_thread_id`.
    pub(crate) async fn shutdown_live_agent_descendants(
        &self,
        parent_thread_id: ThreadId,
    ) -> CodexResult<Vec<ThreadId>> {
        let descendant_ids = self.live_thread_spawn_descendants(parent_thread_id).await?;
        if descendant_ids.is_empty() {
            return Ok(Vec::new());
        }

        let mut shutdown_agent_ids = Vec::new();
        for agent_id in descendant_ids.into_iter().rev() {
            match self.shutdown_live_agent(agent_id).await {
                Ok(_) => shutdown_agent_ids.push(agent_id),
                Err(CodexErr::ThreadNotFound(_)) | Err(CodexErr::InternalAgentDied) => {}
                Err(err) => {
                    warn!("failed to shut down spawned agent {agent_id}: {err}");
                }
            }
        }

        Ok(shutdown_agent_ids)
    }
