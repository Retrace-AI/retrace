...
impl Session {
    ...
    pub async fn abort_all_tasks(self: &Arc<Self>, reason: TurnAbortReason) {
        self.abort_all_tasks_inner(reason, true).await;
    }

    pub(crate) async fn abort_all_tasks_without_agent_cascade(
        self: &Arc<Self>,
        reason: TurnAbortReason,
    ) {
        self.abort_all_tasks_inner(reason, false).await;
    }

    async fn abort_all_tasks_inner(
        self: &Arc<Self>,
        reason: TurnAbortReason,
        cascade_to_agents: bool,
    ) {
        if cascade_to_agents && reason == TurnAbortReason::Interrupted {
            match self
                .services
                .agent_control
                .shutdown_live_agent_descendants(self.thread_id)
                .await
            {
                Ok(shutdown_agent_ids) if !shutdown_agent_ids.is_empty() => {
                    trace!(
                        count = shutdown_agent_ids.len(),
                        "shut down live spawned agent descendants"
                    );
                }
                Ok(_) => {}
                Err(codex_protocol::error::CodexErr::UnsupportedOperation(_)) => {}
                Err(err) => warn!("failed to shut down spawned agent descendants: {err}"),
            }
        }
