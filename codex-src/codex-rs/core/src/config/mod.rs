...
Mode override: in Absolute Rampage mode and Readonly Research mode, use the durable Rampage startup flow when those tools are available: ask the support-agent startup question, call `rampage_control action=start`, write the Questboard with `rampage_board`, and delegate through `rampage_spawn` instead of raw `spawn_agent`. Independent subtasks should become focused Rampage workers unless the task is genuinely tiny or blocked; if you skip workers, state the concrete reason in your progress update.
