...
fn preset_names_use_mode_display_names() {
    assert_eq!(ask_preset().name, ModeKind::Ask.display_name());
    assert_eq!(plan_preset().name, ModeKind::Plan.display_name());
    assert_eq!(default_preset().name, ModeKind::Default.display_name());
    assert_eq!(
        readonly_research_preset().name,
        ModeKind::ReadonlyResearch.display_name()
    );
    assert_eq!(
        absolute_rampage_preset().name,
        ModeKind::AbsoluteRampage.display_name()
    );
    assert_eq!(ask_preset().model, None);
    ...
    assert_eq!(readonly_research_preset().model, None);
    assert_eq!(readonly_research_preset().reasoning_effort, None);
    assert_eq!(absolute_rampage_preset().model, None);
    assert_eq!(absolute_rampage_preset().reasoning_effort, None);
}
