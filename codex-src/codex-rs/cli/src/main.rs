...
#[clap(
    ...
    // The executable can still be invoked through compatibility wrappers, but
    // help output should show this app's user-facing name.
    bin_name = "retrace-tui",
    override_usage = "retrace-tui [OPTIONS] [PROMPT]\n       retrace-tui [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `retrace-tui mcp`: `retrace-tui`, `retrace-tui exec`, `retrace-tui review`, `retrace-tui resume`, `retrace-tui archive`, `retrace-tui unarchive`, `retrace-tui fork`, `retrace-tui mcp`, `retrace-tui sandbox`, and `retrace-tui debug prompt-input`."
        ),
    }
}
...
/// Build the final `TuiCli` for a `retrace resume` invocation.
...
mod tests {
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        ...
        assert!(
            help.contains("Usage: retrace-tui plugin marketplace [OPTIONS] <COMMAND>"),
            "{help}"
        );

        for (subcommand, usage) in [
            ("add", "Usage: retrace-tui plugin marketplace add"),
            ("list", "Usage: retrace-tui plugin marketplace list"),
            ("upgrade", "Usage: retrace-tui plugin marketplace upgrade"),
            ("remove", "Usage: retrace-tui plugin marketplace remove"),
        ] {
    ...
    fn format_exit_messages_includes_resume_hint_without_color() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run retrace resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run retrace resume, then select my-thread (123e4567-e89b-12d3-a456-426614174000)".to_string(),
            ]
        );
    }
    ...
    fn root_strict_config_is_rejected_for_unsupported_subcommands() {
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `retrace-tui mcp`"
        );
